...
void hex_escape_and_print(const char *data, int len, int max_output_bytes)
{
    // Print up to max_output_bytes in hex, then “…” if more.
    int to_show = (len < max_output_bytes ? len : max_output_bytes);
...
// Print current configuration (for the start of the run)
